...
//        filmlists.setOnScrollListener(this);
    ...
    private void getWishlist() {
        ...
        List<Events_Wishlist> wishlist = db.getAllwishlist();
        categoryevents.clear();

        for (int i = 0; i < wishlist.size(); i++) {
            Gson gson = new Gson();
            String list = wishlist.get(i).getDetails();
            Event screens = gson.fromJson(list, Event.class);

            String appurl = wishlist.get(i).getId();

            if (savedirs.contains(appurl))
                screens.setDownloaded(true);
            else
                screens.setDownloaded(false);

            categoryevents.add(screens);
...
//            f_adapter.notifyDataSetChanged();
        ...
        StringRequest jsonRequest = new StringRequest(Request.Method.GET, jsonUrl, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                try {
                    ...
                    boolean sended = LocalBroadcastManager.getInstance(WishList.this).sendBroadcast(new Intent("com.reloadsavedevents"));
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                if (error instanceof TimeoutError) {
                    Error_Dialog.show("Timeout", WishList.this);
                } else if (VolleyErrorLis.isServerProblem(error)) {
                    Error_Dialog.show(VolleyErrorLis.handleServerError(error, context), WishList.this);
                } else if (VolleyErrorLis.isNetworkProblem(error)) {
                    Error_Dialog.show("No Internet Connection", WishList.this);
                }
    ...
    private void openevent(String appid) throws IOException {
        ...
        try {
            ...
            Intent sending = new Intent(WishList.this, SingleEventHome.class);sending.putExtra("Engagement","");sending.putExtra("back","APPS");
...
//        mapFragment = (SupportMapFragment) getSupportFragmentManager().findFragmentById(R.id.map);
    ...
    private void savedeviceapp(final Event events, final int downpos) {
        ...
        final ProgressDialog dialog = new ProgressDialog(WishList.this, R.style.MyAlertDialogStyle);
            ...
            public void onResponse(String response) {
                try {
                    ...
                    if (jObj.getBoolean("response")) {
                        ...
                        Error_Dialog.show(jObj.getString("responseString"), WishList.this);
                    }
...
//                pDialog.dismiss();
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                if (error instanceof TimeoutError) {
                    ...
                    Error_Dialog.show(com.singleevent.sdk.Custom_View.VolleyErrorLis.handleServerError(error, WishList.this), WishList.this);
                } else if (com.singleevent.sdk.Custom_View.VolleyErrorLis.isNetworkProblem(error)) {
                    Error_Dialog.show("Please Check Your Internet Connection", WishList.this);
                }
        ...
        StringRequest strReq = new StringRequest(Request.Method.POST,
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                if (error instanceof TimeoutError) {
                    ...
                    Error_Dialog.show(com.webmobi.gecmedia.CustomViews.VolleyErrorLis.handleServerError(error, WishList.this), WishList.this);
    ...
    private void parseresult(JSONObject details, String appId) throws JSONException {
        ...
        Paper.book().write("SpeakerRating", ratinglist);
                       ...
                       /* setResult(RESULT_OK);
                        finish();*/
        Intent sending = new Intent(this, SingleEventHome.class);sending.putExtra("Engagement","");
    ...
    public void calendarpermission(boolean b, String appID_str, String appTtitle_str, String appStartDate_str, String appEndDate_str, String appDescription_str) {
        if (ActivityCompat.checkSelfPermission(WishList.this, Manifest.permission.READ_CALENDAR) != PackageManager.PERMISSION_GRANTED
                && ActivityCompat.checkSelfPermission(WishList.this, Manifest.permission.WRITE_CALENDAR) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(WishList.this, new String[]{Manifest.permission.READ_CALENDAR, Manifest.permission.WRITE_CALENDAR}, 1);
        } else {
    ...
    private void AdddingTocalendar(boolean b, String appID_str, String appTtitle_str, String appStartDate_str, String appEndDate_str, String appDescription_str) {
        ...
        if (!getcalendarlist.containsKey(appID_str)) {
            ...
            Error_Dialog.show("Successfully added to Calendar", WishList.this);
            getcalendarlist.put(appID_str, appID_str);
            Paper.book().write("CalendarList", getcalendarlist);

        } else {
            Error_Dialog.show("Already added to Calendar", WishList.this);
        }
